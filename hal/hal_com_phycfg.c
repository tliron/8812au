...
)
{
	...
	{
		rtw_merge_string(file_path_bs, PATH_LENGTH_MAX, rtw_phy_file_path, pFileName); // tliron
	
		if (rtw_is_file_readable(file_path_bs) == _TRUE) // tliron
		{
			rlen = rtw_retrive_from_file(file_path_bs, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN); // tliron
			if (rlen > 0)
			{
				rtStatus = _SUCCESS;
